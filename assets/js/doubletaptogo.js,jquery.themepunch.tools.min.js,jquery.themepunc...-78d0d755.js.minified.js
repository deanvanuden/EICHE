...
;(function($){function initEicheColorbox(){if(!$||!$.fn||!$.fn.colorbox){return}var links=$('a[data-lightbox]');if(!links.length){return}links.each(function(){var link=$(this);link.colorbox({rel:link.attr('data-lightbox'),loop:true,slideshow:true,slideshowAuto:false,slideshowSpeed:3600,slideshowStart:'Start Slideshow',slideshowStop:'Stop Slideshow',maxWidth:'92%',maxHeight:'88%',opacity:.84,fixed:true,current:'Bild {current} von {total}',previous:'Vorheriges Bild',next:'Nächstes Bild',close:'Schliessen',onClosed:function(){try{$.colorbox.stop()}catch(e){}},onComplete:function(){var active=$.colorbox.element();var caption=active.attr('title')||active.find('img').attr('alt')||'';$('#cboxTitle').text(caption)}})})}$(function(){setTimeout(initEicheColorbox,0);setTimeout(initEicheColorbox,300)});$(window).on('load',function(){setTimeout(initEicheColorbox,0)})})(jQuery);
